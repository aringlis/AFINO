...
{% for row in site.data.afino_master_record %}
  ...
  <td> {{row.Detection}} </td>
  {% if row.probability_m0 > 1.0 %}
      {%assign prob_m0 = 0.0 %}
  {% else %}
      {% assign prob_m0 = row.probability_m0 %}

  <td> {{prob_m0 | round:3 }}  </td>
